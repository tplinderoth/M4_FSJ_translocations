...
if (parents) {
	...
	external_parents = NULL
	for (id in c(ind.sub$SIRE_ID[which(ind.sub$SIRE_ID != '*')], ind.sub$DAM_ID[which(ind.sub$DAM_ID != '*')])) {
		first_offspring_cohort = ifelse(id %in% ind.sub$SIRE_ID == TRUE, min(ind.sub$COHORT[which(ind.sub$SIRE_ID == id)]), min(ind.sub$COHORT[which(ind.sub$DAM_ID == id)]))
		if (id %in% ind.sub$ID == TRUE && first_offspring_cohort <= ind.sub$COHORT[which(ind.sub$ID == id)]) external_parents = c(external_parents, id)
	}	
	external_parents = unique(c(external_parents, ind.sub$SIRE_ID[which(ind.sub$SIRE_ID %in% ind.sub$ID == FALSE)], ind.sub$DAM_ID[which(ind.sub$DAM_ID %in% ind.sub$ID == FALSE)]))
...
for (time_step in 1:params$n_timesteps) {
	...
	if (length(idx) > 0) {
		...
		anc.id = ind.df[idx,]$ID[which(ind.df$ID[idx] %in% ind.df$ID[which(ind.df$ANCESTOR == 1)] == TRUE)]
	...
	if (length(lranc.idx) > 0) {
		...
		if (length(pairavail) > 0) {
			...
			for (id in names(famtab)) {
				...
				# insert LR ancestors with known parents into pedigree
		...
		if (nrow(ancdf) > 0) {
			# LR ancestors need to be inserted but no breeders are available
	...
	#print(idmap) # debug
	#print(ped) # debug
	#if (length(which(!is.na(ped$ID) & (is.na(ped$SIRE_ID) | is.na(ped$DAM_ID)))) > 0) stop(print(time_step)) # debug to ensure that offspring have assigned parents
...
# format pedigree
ped = ped[,c(1:6)]
empty_idx = which(is.na(ped$COHORT))
if (length(empty_idx)> 0) ped = ped[-empty_idx,] # trim away empty buffer rows
